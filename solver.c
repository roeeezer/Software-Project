...
ERROR autofillBoard(board* b,board* bt,moveNode* move,int ind,int gameMode){
	int* valuesList;
	...
	for(i=0;i<N;i++){
		for(j=0;j<N;j++){
			...
			if(v!=0){
				if(ind==1){
					setCellUpdateErroneousAndMove(b,bt,move, i, j,v,gameMode);}
				else{
					setCellAndUpdateErroneous(b,bt, i, j,v,gameMode);
				}
			}
		}}
	free(valuesList);
